...
def emit_decl_tex(decl: DeclInfo, extracted: dict, labels: dict[str, str], selected: set[str]) -> str:
    env_name = {
        "def": "definition",
    ...
    stmt = [labels[d] for d in extracted["statementDeps"] if d in selected]
    body = [labels[d] for d in extracted["bodyDeps"] if d in selected]
...
def write_blueprint(
    targets: list[tuple[str, Path]], decls: list[DeclInfo], extracted_rows: list[dict], *, nonreduced: bool
) -> None:
    ...
    selected = {d.name for d in decls}
    labels = {d.name: sanitize_label(d.kind, d.name) for d in decls}
    ...
    for module, _ in targets:
        ...
        for decl in by_module.get(module, []):
            module_lines.append(emit_decl_tex(decl, extracted[decl.name], labels, selected))
            module_lines.append("")
...
def restyle_dot(dot_path: Path) -> None:
    ...
    for part in parts:
        ...
        if "->" in stripped:
            ...
            target_prefix = classify_node_prefix(target_id)
            if target_prefix in {"def:", "str:", "inst:"}:
                attr_map.pop("style", None)
            elif target_prefix in {"lem:", "thm:"}:
                if attr_map.get("style") == "dashed":
                    attr_map.pop("style", None)
                else:
                    attr_map["style"] = "dashed"
            attr_text = ""
            if attr_map:
    ...
    legend = (
        ...
        'fontsize=18;'
        'color="#888888";'
        '"legend_def" [label="definition", shape=box, style="rounded,filled", color=black, fillcolor="#d9ead3"];'
        '"legend_str" [label="structure", shape=box, style="rounded,filled", color=black, fillcolor="#e8f0ff"];'
        '"legend_lem" [label="lemma", shape=ellipse, style=filled, color=black, fillcolor="#fff2cc"];'
        '"legend_thm" [label="theorem", shape=ellipse, style=filled, color=black, fillcolor="#f4cccc"];'
        '"legend_stmt" [label="statement dependency:  --------->", shape=plaintext];'
        '"legend_proof" [label="proof dependency:  -- -- -- >", shape=plaintext];'
        '{rank=same; "legend_def"; "legend_str"; "legend_lem"; "legend_thm";}'
        '{rank=same; "legend_stmt"; "legend_proof";}'
        '}'
    )
    text = text.rstrip()
    if text.endswith("}"):
